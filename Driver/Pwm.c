	...
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)  //检查TIM3更新中断发生与否
		{
				...
				//main_obj.timer_count++;
				main_obj.send_count++;
				main_obj.state_counter++;
				if(main_obj.send_count == 5)
				{
    					send_data();
					main_obj.send_count = 0;
				}
				main_obj.fliter_flag = 1;
				wd_flag = 1;
				if( main_obj.state_counter >10000 )
				{
					main_obj.state_counter = 10000;
				}
		}
}
